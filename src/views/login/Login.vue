    ...
    <a-form :form="form" @submit="handleSubmit">
      <a-tabs :tab-bar-style="{textAlign:'center'}" :activeKey="activeKey" @change="handleTabChange">
        <a-tab-pane tab="账户密码登录" key="1">
          <a-alert type="error" closable v-show="error" :message="error" showIcon style="margin-bottom: 24px;"></a-alert>
          <a-form-item>
            <a-input v-decorator="['username',{rules:[{required:true,message:'请输入账户名',whitespace:true}]}]" size="large" placeholder="用户名">
              <a-icon slot="prefix" type="user" />
            </a-input>
          </a-form-item>
          <a-form-item>
            <a-input v-decorator="['password',{rules:[{required:true,message:'请输入密码',whitespace:true}]}]" size="large" type="password" placeholder="密码">
